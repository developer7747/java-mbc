	...
	<div class="container">
      <c:forEach items="${listStatebyName}" var="State">
			<c:forEach items="${listSpecialitybyName}" var="Specialityies">
				<div class="col-sm-7">
			<h3>
						<label id="ctl00_ContentPlaceHolder1_heading_title"></label>
					</h3>
				<p class="MsoNormal">${State.bannerContent}
				
				
<%-- 						${Specialityies.bannerContent} ${Specialityies.catname} --%>

<%-- 						${Specialityies.customName} ${State.bannerContent} --%>
<%-- 						${State.imagepath} ${Specialityies.imagepath} --%>
<%-- 						${Specialityies.name} ${State.name} --%>
</p>
				</div>
				<div class="col-sm-5">
